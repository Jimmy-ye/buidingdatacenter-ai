...
```bash
# Windows - 一键启动所有服务
scripts\快速启动\一键启动所有服务.bat
```

**说明**:
- 自动启动后端（8000）、Worker（8001）、PC-UI（8080）
- 每个服务在独立窗口运行
- 自动打开浏览器
...
```bash
scripts\快速启动\启动PC-UI.bat
```

#### 启动 Worker

```bash
scripts\快速启动\启动Worker.bat
