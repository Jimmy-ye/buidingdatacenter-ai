...
from shared.db.models_project import Building, Zone, BuildingSystem, Device, Project
...
async def create_building_for_project(
    ...
    """Create a building and optionally attach default systems.

    根据 docs/02-技术文档/工程结构API设计.md 中的「1.4 默认系统模板」约定，
    在创建 Building 后自动在该楼栋下创建一批基础系统，以降低工程录入成本。
    ...
    # 自动创建默认系统模板
    default_systems = [
        {"type": "envelope", "name": "围护结构"},
        {"type": "cooling", "name": "制冷"},
        {"type": "heating", "name": "制热"},
        {"type": "terminal_hvac", "name": "空调末端"},
        {"type": "lighting", "name": "照明"},
        {"type": "elevator", "name": "电梯"},
        {"type": "power", "name": "动力"},
        {"type": "ems", "name": "电力监控"},
        {"type": "energy_platform", "name": "能管平台"},
    ]

    for tpl in default_systems:
