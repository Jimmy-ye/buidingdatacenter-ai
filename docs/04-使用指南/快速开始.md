# BDC-AI 快速开始

## 📖 文档说明

本指南将帮助你在 5 分钟内快速启动并体验 BDC-AI 系统。

**适用人群**:
- 新成员入职
- 项目干系人了解系统
- 快速体验核心功能

**时间估算**: 5 分钟

---

## ✅ 前置要求

### 环境要求

- **Python**: 3.11 或更高版本
- **PostgreSQL**: 18.1 或更高版本
- **操作系统**: Windows / macOS / Linux
- **内存**: 至少 4GB RAM
- **磁盘**: 至少 1GB 可用空间

### 检查环境

```bash
# 检查 Python 版本
python --version

# 检查 PostgreSQL 版本
psql --version

# 检查 pip
pip --version
```

---

## 🚀 5 分钟快速体验

### 步骤 1：克隆项目（1 分钟）

```bash
# 克隆项目
git clone https://github.com/your-org/program-bdc-ai.git

# 进入项目目录
cd program-bdc-ai
```

### 步骤 2：配置环境（1 分钟）

```bash
# 复制配置模板
cp .env.example .env

# 编辑 .env 文件，填入实际配置
# Windows: notepad .env
# macOS/Linux: nano .env
```

**必需配置**:
```bash
# 数据库连接 URL
BDC_DATABASE_URL=postgresql://admin:password@localhost:5432/bdc_ai

# JWT 密钥（生产环境使用 openssl rand -hex 32 生成）
BDC_JWT_SECRET_KEY=your-secret-key-change-in-production

# GLM API Key（用于 AI 分析）
GLM_API_KEY=your_glm_api_key_here
```

### 步骤 3：安装依赖（1 分钟）

```bash
# 安装后端依赖
pip install -r services/backend/requirements.txt

# 验证安装
python -c "import fastapi; import sqlalchemy; print('依赖安装成功！')"
```

### 步骤 4：初始化数据库（1 分钟）

```bash
# 创建数据库
psql -U postgres
CREATE DATABASE bdc_ai OWNER admin;
GRANT ALL PRIVILEGES ON DATABASE bdc_ai TO admin;
\q

# 初始化认证系统
python scripts/init_auth_db.py

# 验证初始化
# 应该看到：创建 27 个权限、6 个角色、4 个用户的提示
```

### 步骤 5：启动服务（1 分钟）

```bash
# 启动后端服务
python -m uvicorn services.backend.app.main:app --host localhost --port 8000 --reload

# 看到以下提示表示启动成功：
# INFO:     Uvicorn running on http://localhost:8000 (Press CTRL+C to quit)
```

### 步骤 6：访问 API 文档

打开浏览器访问：
- **Swagger UI**: http://localhost:8000/docs
- **ReDoc**: http://localhost:8000/redoc

---

## 🎯 第一个 API 调用

### 登录获取 Token

使用默认账号 `admin/admin123` 登录：

**使用 cURL**:
```bash
curl -X POST http://localhost:8000/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"admin123"}'
```

**使用 Swagger UI**:
1. 访问 http://localhost:8000/docs
2. 找到 `/api/v1/auth/login` 端点
3. 点击 "Try it out"
4. 输入用户名和密码
5. 点击 "Execute"
6. 复制返回的 `access_token`

**响应示例**:
```json
{
  "access_token": "eyJ0eXAiOiJKV1QiLCJhbGc...",
  "refresh_token": "eyJ0eXAiOiJKV1QiLCJhbGc...",
  "token_type": "bearer",
  "user": {
    "id": "...",
    "username": "admin",
    "email": "admin@example.com",
    "role": "superuser"
  }
}
```

### 创建第一个项目

使用获取的 Token 创建项目：

**使用 cURL**:
```bash
curl http://localhost:8000/api/v1/projects/ \
  -H "Authorization: Bearer <your_access_token>" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "我的第一个项目",
    "client": "测试客户",
    "location": "北京",
    "type": "public_building",
    "status": "planning"
  }'
```

**使用 Swagger UI**:
1. 点击右上角 "Authorize" 按钮
2. 输入 `Bearer <your_access_token>`（注意 Bearer 后有空格）
3. 点击 "Authorize"
4. 找到 `/api/v1/projects/` 端点
5. 点击 "Try it out"
6. 输入项目信息
7. 点击 "Execute"

**响应示例**:
```json
{
  "id": "...",
  "name": "我的第一个项目",
  "client": "测试客户",
  "location": "北京",
  "type": "public_building",
  "status": "planning",
  "created_at": "2026-01-25T10:00:00Z"
}
```

---

## 🔐 修改默认密码

⚠️ **重要**: 登录后立即修改默认密码！

**使用 cURL**:
```bash
curl -X POST http://localhost:8000/api/v1/auth/me/change-password \
  -H "Authorization: Bearer <your_access_token>" \
  -H "Content-Type: application/json" \
  -d '{
    "old_password": "admin123",
    "new_password": "your_new_secure_password"
  }'
```

**使用 Swagger UI**:
1. 找到 `/api/v1/auth/me/change-password` 端点
2. 点击 "Try it out"
3. 输入旧密码和新密码
4. 点击 "Execute"

---

## 📱 下一步体验

### 体验项目管理

- 创建多个项目
- 查询项目列表
- 更新项目信息
- 删除项目（支持软删除和恢复）

### 体验工程结构

- 创建建筑（Building）
- 创建区域（Zone）
- 创建系统（System）
- 创建设备（Device）

### 体验多模态资产

- 上传图片资产
- 查看 OCR 识别结果
- 查看 GLM-4V 分析结果
- 上传表格资产

### 体验认证系统

- 创建新用户
- 分配角色和权限
- 测试不同角色的权限
- 查看审计日志

---

## 🛠️ 常见问题

### 问题 1：数据库连接失败

**错误信息**:
```
sqlalchemy.exc.OperationalError: (psycopg2.OperationalError) could not connect to server
```

**解决方案**:
1. 检查 PostgreSQL 服务是否运行
   ```bash
   # Windows
   sc query postgresql-x64-18

   # macOS/Linux
   sudo systemctl status postgresql
   ```

2. 检查 `.env` 中的数据库 URL 是否正确
   ```bash
   BDC_DATABASE_URL=postgresql://admin:password@localhost:5432/bdc_ai
   ```

3. 确认数据库已创建
   ```bash
   psql -U postgres -l
   ```

### 问题 2：依赖安装失败

**错误信息**:
```
ERROR: Could not find a version that satisfies the requirement ...
```

**解决方案**:
1. 升级 pip
   ```bash
   pip install --upgrade pip
   ```

2. 使用国内镜像源
   ```bash
   pip install -r services/backend/requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
   ```

### 问题 3：JWT 密钥错误

**错误信息**:
```
jose.exceptions.JWSError: Not enough segments
```

**解决方案**:
检查 `.env` 中的 JWT 密钥是否设置：
```bash
BDC_JWT_SECRET_KEY=your-secret-key-change-in-production
```

生成安全的密钥：
```bash
# macOS/Linux
openssl rand -hex 32

# Python
python -c "import secrets; print(secrets.token_hex(32))"
```

### 问题 4：端口被占用

**错误信息**:
```
OSError: [Errno 48] Address already in use
```

**解决方案**:
1. 更换端口
   ```bash
   python -m uvicorn services.backend.app.main:app --host localhost --port 8001 --reload
   ```

2. 或者停止占用 8000 端口的进程
   ```bash
   # 查找占用端口的进程
   lsof -i :8000

   # 停止进程
   kill -9 <PID>
   ```

### 问题 5：认证失败

**错误信息**:
```
{"detail": "Incorrect username or password"}
```

**解决方案**:
1. 确认使用了正确的用户名和密码
   - 默认账号: `admin` / `admin123`

2. 重新初始化数据库
   ```bash
   python scripts/init_auth_db.py
   ```

---

## 📚 进阶学习

### 完整认证系统指南

👉 **[账号权限系统使用指南.md](./账号权限系统使用指南.md)**

包含内容：
- Web API 使用
- Python 脚本调用
- Flutter 移动端集成
- NiceGUI PC 端集成
- 用户和权限管理
- 常见场景示例

### API 开发指南

👉 **[后端API开发指南.md](./后端API开发指南.md)**（待创建）

### 部署运维指南

👉 **[部署运维指南.md](./部署运维指南.md)**（待创建）

---

## 🎉 恭喜

你已经成功完成了 BDC-AI 的快速体验！

**你学会的技能**:
- ✅ 安装和配置 BDC-AI
- ✅ 启动后端服务
- ✅ 调用认证 API
- ✅ 创建项目
- ✅ 修改密码

**下一步建议**:
1. 阅读 **[账号权限系统使用指南.md](./账号权限系统使用指南.md)** 深入学习认证系统
2. 阅读 **[后端API开发指南.md](./后端API开发指南.md)** 学习 API 开发（待创建）
3. 阅读 **[项目总结.md](../00-项目总览/项目总结.md)** 了解项目全貌

---

## 📞 获取帮助

遇到问题？

1. **查看文档**: [docs/](../)
2. **提交 Issue**: 在项目仓库提交 Issue
3. **联系团队**: 联系 BDC-AI 开发团队

---

**文档版本**: 1.0.0
**最后更新**: 2026-01-25
**维护者**: BDC-AI 开发团队
