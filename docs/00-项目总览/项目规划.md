...
- **Building（建筑）**
  - 字段：`id`, `project_id`, `name`, `usage_type`（办公、商场…）, `floor_area`, `gfa_area`, `year_built`.
...
### 6.3 前端 UI 实现进度（2026-01-22）

**PC Web UI（NiceGUI）**

- 已完成：
  - 工程结构树浏览：项目 → 楼栋 → 系统 → 设备（基于 `/api/v1/projects/{project_id}/structure_tree`）。
  - 工程结构编辑：新建项目、新建楼栋。
  - 设备资产管理：按设备查看图片资产，支持上传与删除。
  - 图片 OCR 管线：`/api/v1/assets/{asset_id}/parse_image` → `image_annotation` → 在 PC 端展示 OCR 文本和统计信息。
  - 场景问题 LLM 管线：`/api/v1/assets/{asset_id}/route_image` + GLM Worker → `scene_issue_report_v1` → 在 PC 端展示问题摘要与建议。

- 待实现：
  - 工程结构编辑：新建系统、新建设备（后端 CRUD API 已就绪，UI 待接入）。
  - 铭牌（`nameplate`）参数表结构化解析及表格展示。
  - 表格模态解析与统计可视化页面。
  - 综合诊断大盘与多模态结果汇总。
- 能看到某设备的所有 OCR / 场景报告
