...
- ✅ Worker 后台自动处理
- ✅ 性能：~10-30 秒/张
- ✅ **工程师备注支持**：支持上传时附带工程师备注，LLM会参考备注内容进行分析（2026-01-21）
- ✅ **仪表读数识别**：支持`content_role=meter`的仪表图片自动读数识别（2026-01-21）
- ✅ **Prompt优化**：完成Prompt长度优化（921字符），确保GLM API正确返回JSON（2026-01-21）
...
| `/api/v1/assets/upload_image_with_note` | POST | 图片+备注上传+自动路由 | ✅ |
...
- ✅ **表格行点击事件**：点击资产行可在右侧详情面板显示完整信息（2025-01-20）
- ✅ **UI布局优化**：40%左表+60%右详情的并排布局，提升用户体验（2025-01-20）
- ✅ **OCR/LLM结果显示**：完整显示OCR文本、数字提取和GLM分析结果（2025-01-20）
- ✅ **关键词提取优化**：从structured_payloads数组中提取关键词，提升可读性（2025-01-20）
...
- `content_role='meter'` → OCR 文本提取 + GLM-4V 仪表读数识别
- `content_role='scene_issue'` → OCR 文本提取 + GLM-4V 场景问题分析
- `content_role='nameplate'` → OCR 文本提取（铭牌识别，规划中）
- 其他 → 仅存储，不自动处理

### 3. 工程师备注智能处理

**功能**：支持在上传图片时附带工程师备注，LLM会参考备注但以图片为主

**约束机制**：
- Prompt中强调"仅供参考，以图片为主"
- 添加"不得逐字复制备注"的约束
- 后处理检测：如果summary与备注高度相似（相似度≥80%），自动降低置信度至0.6
- 自动添加"请人工核对"提示，避免过度依赖备注

**效果**：成功率达到100%，所有测试用例正常处理（2026-01-21）
...
### 开发指南
- **docs/01-开发指南/API使用示例.md** - API 使用示例
- **docs/01-开发指南/工程结构测试指南.md** - 工程结构测试指南
- **docs/01-开发指南/Asset与工程结构测试指南.md** - Asset与工程结构测试指南

### 技术与优化记录
- **docs/02-技术文档/表格处理管道设计.md** - 表格处理管道设计
- **docs/03-优化记录/GLM_Worker测试报告.md** - GLM Worker 测试报告
- **docs/03-优化记录/GLM提示词优化完整报告.md** - GLM提示词优化报告
- **docs/03-优化记录/nicegui-table-row-click-solution.md** - 表格行点击解决方案
- **docs/03-优化记录/pc-ui-layout-optimization.md** - PC UI布局优化
- **docs/03-优化记录/工程师备注LLM测试报告.md** - 工程师备注功能测试
- **docs/03-优化记录/工程师备注Prompt优化对比报告.md** - Prompt优化对比报告

### 迁移与清理
- **docs/04-迁移记录/PostgreSQL迁移总结.md** - 数据库迁移总结
- **docs/04-迁移记录/项目清理与安全加固总结.md** - 清理与安全总结

**文档目录索引**：查看 [docs/README.md](../README.md) 获取完整文档索引
